...
class Ui_MainWindow(object):
    ...
    def initialize(self): 
        ...
        self.lineEdit_colorRange.setText("0.01, 0.1")
    ...
    def plotting(self, nodeplotRecursive=False): 
        ...
        c_range =  self.lineEdit_colorRange.text().strip()
        cr = c_range.split(",")
        try: 
            low = float(cr[0].strip()); high = float(cr[1].strip())
        except Exception as ex: 
            print (ex)
            print ('Check color range')
            self.lineEdit_colorRange.setText("0.01, 0.1")
            low = 0.01; high = 0.1
            ...
            if not self.xClipped and not self.yClipped and not self.zClipped: 
                for surface, press, clr in zip(self.surfaces, self.presses, self.colors): 
                    ...
                    if self.checkBox_Slicing.isChecked(): 
                        ...
                        # print (" Color Range %d~%d"%(start, end))
                        # dargs = dict(scalars=item, cmap='rainbow', show_edges=self.show_meshLine, n_colors=150, clim=[start, end])
                    ...
                    if self.radioButton_cPres.isChecked():
                        ...
                        # colormap, scalars = self.defineColorMap(press, item, vmin, vmax, div, isSurf=True)
                        ...
                        print (" Color Range %d~%d"%(start, end))
                        dargs = dict(scalars=item, cmap='rainbow', show_edges=self.show_meshLine, n_colors=150, clim=[start, end])
                        self.plotter_qulity = self.plotter.add_mesh(press, interpolate_before_map=True, opacity=self.opecity, 
                             scalar_bar_args={'title': ' %s  - interpolated'%(item)}, **dargs)
                ...
                for surface, press, clr in zip(self.surfaces, self.presses, self.colors): 
                    ...
                    self.plotter_surf = self.plotter.add_mesh(clippedMesh, show_edges=self.show_meshLine, color=clr, metallic=0.3, pbr=False, diffuse=1, opacity=self.opecity, smooth_shading=False) 
                    ...
                    if self.radioButton_cPres.isChecked():
                        try: 
                            vmax = clippedPress[item].max(); vmin = clippedPress[item].min()
                            # colormap, scalars = self.defineColorMap(clippedPress, item, vmin, vmax, div, isSurf=True)
                            # psize = 5# float(psize)
                            # self.plotter_qulity = self.plotter.add_mesh(clippedPress, scalars=scalars, cmap=colormap, point_size=psize, render_points_as_spheres=True)

                            vgap = vmax - vmin  
                            start = vmin + vgap * low; end = vmin + vgap * high
                            print (" Color Range %d~%d"%(start, end))
                            dargs = dict(scalars=item, cmap='rainbow', show_edges=self.show_meshLine, n_colors=150, clim=[start, end])
                            self.plotter_qulity = self.plotter.add_mesh(clippedPress, interpolate_before_map=True, opacity=self.opecity, 
                                scalar_bar_args={'title': ' %s  - interpolated'%(item)}, **dargs)
            ...
            if not self.xClipped and not self.yClipped and not self.zClipped: 
                ...
                for mesh, clr, edge, surface in zip(self.meshes, self.colors, self.edges, self.surfaces):
                    ...
                    if self.radioButton_SDB_sed.isChecked() or self.radioButton_SDB_eld.isChecked(): 
                        ...
                        # colormap, scalars = self.defineColorMap(mesh, item, vmin, vmax, div, isSurf=False)
                        # self.plotter_qulity = self.plotter.add_mesh(mesh, scalars=scalars, cmap=colormap, show_edges=self.show_meshLine, pbr=False, diffuse=1, opacity=self.opecity )
                        ...
                        # self.plotter_qulity = self.plotter.add_mesh_slice(mesh, interpolate_before_map=True, color=True, opacity=1.0, 
                        #         scalar_bar_args={'title': ' %s  - interpolated'%(item)}, **dargs)
                        self.plotter.enable_cell_picking(mesh=mesh)
                    elif self.radioButton_Temperature.isChecked(): 
                        vmax = self.temperatures[cnt][item].max(); vmin = self.temperatures[cnt][item].min()
                        vgap = vmax - vmin  
                        self.lineEdit_colorRange.setText("0.1, 0.95")
                        low = 0.1; high = 0.95
                        start = vmin + vgap * low; end = vmin + vgap * high
                        print (" Color Range %d~%d"%(start, end))
                        dargs = dict(scalars=item, cmap='rainbow', show_edges=self.show_meshLine, n_colors=150, clim=[start, end])
                        self.plotter_qulity = self.plotter.add_mesh(self.temperatures[cnt], interpolate_before_map=True, opacity=self.opecity, 
                             scalar_bar_args={'title': ' %s  - interpolated'%(item)}, **dargs)
                        if self.checkBox_Slicing.isChecked(): 
                            angle = float(self.lineEdit_slicingAngle.text().strip())
                            radian_angle = math.radians(angle)
                            try: 
                                sliced = self.temperatures[cnt].slice(normal=[math.cos(radian_angle), 0, math.sin(radian_angle)])
                                self.plotter_qulity = self.plotter.add_mesh(sliced, interpolate_before_map=True, opacity=1.0, 
                                    scalar_bar_args={'title': ' %s  - interpolated'%(item)}, **dargs)
                            except Exception as ex: 
                                print(ex)
                        ...
                        if self.show_qualityCheck:        
                            ...
                            self.plotter_qulity = self.plotter.add_mesh(showmesh, show_edges=self.show_meshLine, pbr=False, diffuse=1, opacity=self.opecity )
                        elif self.solidClip :             
                            showmesh = mesh 
                            self.plotter_qulity = self.plotter.add_mesh(showmesh, color=clr, show_edges=self.show_meshLine, pbr=False, diffuse=1, opacity=self.opecity )
                        else:                             
                            showmesh = surface 
                            self.plotter_qulity = self.plotter.add_mesh(showmesh, color=clr, show_edges=self.show_meshLine, pbr=False, diffuse=1, opacity=self.opecity )
                ...
                for mesh, clr, edge, surface in zip(self.meshes, self.colors, self.edges, self.surfaces):
                    try: 
                        ...
                        if self.radioButton_SDB_sed.isChecked() or self.radioButton_SDB_eld.isChecked():  
                            ...
                            vmax = clippedMesh[item].max(); vmin = clippedMesh[item].min()
                            # colormap, scalars = self.defineColorMap(clippedMesh, item, vmin, vmax, div, isSurf=False)
                            # self.plotter_qulity = self.plotter.add_mesh(clippedMesh, scalars=scalars, cmap=colormap, show_edges=self.show_meshLine, pbr=False, diffuse=1, opacity=self.opecity )
                            ...
                            clippedMesh = self.clipping(self.temperatures[cnt])
                            vmax = clippedMesh[item].max(); vmin = clippedMesh[item].min()
                            vgap = vmax - vmin  
                            self.lineEdit_colorRange.setText("0.1, 0.95")
                            low = 0.1; high = 0.95
                            ...
                            if self.show_qualityCheck:
                                ...
                                self.plotter_qulity = self.plotter.add_mesh(clippedMesh, show_edges=self.show_meshLine, pbr=False, diffuse=1, opacity=self.opecity )
                            elif self.solidClip :   
                                clippedMesh = self.clipping(mesh)          
                                self.plotter_qulity = self.plotter.add_mesh(clippedMesh, color=clr, show_edges=self.show_meshLine, pbr=False, diffuse=1, opacity=self.opecity )
                            else:               
                                clippedMesh = self.clipping(surface)              
                                self.plotter_qulity = self.plotter.add_mesh(clippedMesh, color=clr, show_edges=self.show_meshLine, pbr=False, diffuse=1, opacity=self.opecity )
                ...
                if self.checkBox_Slicing.isChecked(): 
                    if '.ptn' in self.meshfile: 
                        ...
                        if self.show_qualityCheck:
                            ...
                            self.plotter_qulity = self.plotter.add_mesh(sliced, opacity=1.0, show_edges=True, colors=True, edge_color='gray')
                        elif self.solidClip: 
                            sliced = mesh.slice(normal=[math.cos(radian_angle), 0, math.sin(radian_angle)])
                            self.plotter_qulity = self.plotter.add_mesh(sliced, opacity=1.0, show_edges=True, color=clr, edge_color='gray')
                        else: 
                            sliced = surface.slice(normal=[math.cos(radian_angle), 0, math.sin(radian_angle)])
                ...
                if self.checkBox_Slicing.isChecked(): 
                    if '.ptn' in self.meshfile: 
                        if self.show_qualityCheck:
                            ...
                            self.plotter_qulity = self.plotter.add_mesh_slice(clippedMesh, color=clr, opacity=1.0, show_edges=True)
                        else: 
