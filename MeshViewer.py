...
class Ui_MainWindow(object):
    ...
    def RegenMesh(self): 
        ...
        if self.savefile:
            ...
            if len(self.node.Node[0]) > 4 : self.checkBox_Temperaure.setEnabled(True)
            else: self.checkBox_Temperaure.setEnabled(False)
            self.list_widget.clear()
            self.ElsetNames=[]
    ...
    def add_results(self): 
        ...
        if self.readSDB and self.meshfile: 
            if self.checkBox_Temperaure.isChecked():
                ...
                # print (" not supported !! ")
                return 
                if len(self.E_node.Node) == 0:
                    tempfile = 'temp-regen.tmp'
                    tieErr = RegenenerateMesh(tempfile, self.meshfile, self.node, self.element, self.elset)
                    self.node = NODE()
                    self.element = ELEMENT()
                    self.elset = ELSET()
                    self.surface = []
                    ...
                    self.node, self.element, self.elset, self.surface, self.tie, self.xy, self.rims = Mesh2DInformation(tempfile, comments=False)
                    ...
                    for el in self.element.Element:
                        ...
                        sum = 0 
                        ix = np.where(self.Eloss[:,0]-td % self.limit_offset == el[0])[0]
                        for x in ix: 
                            sum+=self.Eloss[x][1]
                        eld = sum / 2/3.141592 / el[7] / el[9]
                        el.append(eld)
                    

                    values = np.array(self.E_node.Node, dtype=np.float64)
                    print ("\n Min=%.1f, Max=%.1f, average=%.1f"%(np.min(values[:,4]), np.max(values[:,4]), np.average(values[:,4])))
                    # print ("Energy loss density", len(self.E_node.Node))
                    std = np.std(values[:,4])
                    avg =  np.average(values[:,4])
                    print ("Standard Deviation=%.1f"%(std))
                    self.figure.AddNodes(self.E_node, 10, color=1, vmin=avg - std*0.5, vmax=avg + std*0.5 )
                # print ("Energy loss density Done!")

            elif self.checkBox_SED.isChecked():
                # print (" not supported !! ")
                return 
                if len(self.S_node.Node) ==0:  
                    # cnt = 0 
                    # print ("SED Collecting: ")
                    for el in self.element.Element:
                        if el[3] ==0: continue  
                        if el[5] == 'CTB' or el[5] == 'CTR' or el[5] == 'SUT' or el[5] == 'UTR': 
                            td =10**7
                        ...
                        sum = 0 
                        ix = np.where(self.SED[:,0]-td  % self.limit_offset == el[0])[0]
                        for x in ix: 
                            sum+=self.SED[x][1]
                        sed = sum / 2/3.141592 / el[7] / el[9]
                        self.S_node.Node.append([el[0], 0, el[8], el[9], sed])
                        # cnt +=1
                        # if not int(cnt) %100: print ("%d, "%(cnt), end="")
                values = np.array(self.S_node.Node)
                print ("\n Min=%.1f, Max=%.1f, average=%.1f"%(np.min(values[:,4]), np.max(values[:,4]), np.average(values[:,4])))
                # print ("Strain Energy density", len(self.S_node.Node))
                std = np.std(values[:,4])
                avg =  np.average(values[:,4])
                print ("Standard Deviation=%.1f"%(std))
                self.figure.AddNodes(self.S_node, 10, color=1, vmin=avg - std*0.5, vmax=avg + std*0.5 )
                # print ("Strain Energy density Done!")
            else: 
                self.Redraw()
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.checkBox_EnergyLoss.setText(_translate("MainWindow", "E-Loss"))
...
class myCanvas(FigureCanvas):
    ...
    def plot_temperature(self, npn, els, vmin=25, vmax=100, levels=10, contour=None, size=0):
        self.clearplot()
