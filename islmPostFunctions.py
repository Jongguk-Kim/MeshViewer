...
def NormalVectorOfVectors(v1, v2): 
    N1 = v1[2]*v2[3] - v1[3]*v2[2]
    N2 = v1[3]*v2[1] - v1[1]*v2[3]
    N3 = v1[1]*v2[2] - v1[2]*v2[1]
...
def DRRonDrum(lastSDBresultfile, SimTime, SimCond, offset=10000, tread=10000000, **args):
    ...
    npn1 = np.array(Node1.Node)
    npn2 = np.array(Node2.Node)
    npn3 = np.array(Node3.Node)
    
    Det = 0 
    cnt = 0 
    while Det ==0: 
        n2Max = np.max(npn2[:,3])
        ix = np.where(npn2[:,3]==n2Max)[0][0]
        
        Det = (npn2[ix][1]-npn1[ix][1])*(npn3[ix][3]-npn2[ix][3])-(npn2[ix][3]-npn1[ix][3])*(npn3[ix][1]-npn2[ix][1])
        A = npn2[ix][1] * npn2[ix][1] - npn1[ix][1] * npn1[ix][1] + npn2[ix][3] * npn2[ix][3] - npn1[ix][3] * npn1[ix][3]
        B = npn3[ix][1] * npn3[ix][1] - npn2[ix][1] * npn2[ix][1] + npn3[ix][3] * npn3[ix][3] - npn2[ix][3] * npn2[ix][3]
        cnt += 1
        if cnt > 1000: break 

        itop = int(npn2[ix][0])
        if Det ==0: 
            if cnt%100==0: print ("%d, DET %f, Delete Node %d"%(cnt, Det, npn2[ix][0]))
            npn1 = np.delete(npn1, ix, axis=0)
            npn2 = np.delete(npn2, ix, axis=0)
            npn3 = np.delete(npn3, ix, axis=0)
        
    for i, nd in enumerate(Node2.Node): 
        if nd[0] == itop: 
            iTop = i 
    ...
    try:
        ...
        print ("DRR Cannot be calculated on Flat surface at here. Use function DRRonRoad instead")
        DRR = 0.0
...
def DRRonRoad(lastSDBresultfile, SimTime, SimCond, offset=10000, tread=10000000, **args):
    ...
    serial = int(lastSDBresultfile[-3:])
    print ("** DRR on Road ")
    lastSDBresultfile = strSDB+str(format(serial-step*2, '03'))
    Node1 = ResultSDB(strSDB, lastSDBresultfile, offset, tread, 1, 0)
    Rim1  = GetRimCenter(lastsdb=lastSDBresultfile)
    lastSDBresultfile = strSDB+str(format(serial-step, '03'))
    Node2 = ResultSDB(strSDB, lastSDBresultfile, offset, tread, 1, 0)
    Rim2  = GetRimCenter(lastsdb=lastSDBresultfile)
    lastSDBresultfile = strSDB+str(format(serial, '03'))
    Node3 = ResultSDB(strSDB, lastSDBresultfile, offset, tread, 1, 0)
    ...
    DelT = SimTime.DelTime * step * 2 
    
    DRR = SimCond.Speed/3.6*DelT/ Angle 
    if debug: 
        print ("%8d, %10.6f, %10.6f"%(Node1.Node[iTop][0], Node1.Node[iTop][1], Node1.Node[iTop][3]))
        print ("%8d, %10.6f, %10.6f"%(Node2.Node[iTop][0], Node2.Node[iTop][1], Node2.Node[iTop][3]))
        print ("%8d, %10.6f, %10.6f"%(Node3.Node[iTop][0], Node3.Node[iTop][1], Node3.Node[iTop][3]))
        print ("cnt, %10.6f, %10.6f"%(Xc, Yc))
        print ("dst, %10.6f, %10.6f, %10.6f"%(math.sqrt((Node1.Node[iTop][1]-Xc)**2 + (Node1.Node[iTop][3]-Yc)**2), math.sqrt((Node2.Node[iTop][1]-Xc)**2 + (Node2.Node[iTop][3]-Yc)**2), math.sqrt((Node3.Node[iTop][1]-Xc)**2 + (Node3.Node[iTop][3]-Yc)**2)))
        print (" step=%d, Del Time=%.2f, Angle=%.3f"%(step, DelT, math.degrees(Angle)))
    return DRR 
...
def plot_DetailLayout(filename, Element, Node, dpi=200, AddingNodes=[], rim=[], group='PCR', beadring='Tubeless', writeProfile=0):# , Elset):
    ...
    plt.text(textX+0.01, textY, "W:%.2f"%(textX*2000), fontsize=6)
...
def Plot_TemperatureDotting(ImageName, TNode, Element, dpi=200, Offset = 10000, **args): 
    ...
    for key, value in args.items():
        ...
        if key == "size":
            size = value 
        if key == "dots": 
            points = value  ## numpy array , dots[0] =[id, x, y, z, temperature]
    ...
    if len(points) > 0 : 
        ...
        px = np.concatenate((px, pxt))
        py = np.concatenate((py, pyt))
        pv = np.concatenate((pv, pvt))
...
def ValueInterpolation (dots, E, N, XY=21, vmin='', vmax='', **args):
    ...
    for el in E.Element:
        if el[6] == 2: continue
            ...
            N3 = N.NodeByID(el[3])
            N4 = N.NodeByID(el[4])
            if N1[0] ==0 or N2[0] ==0 or N3[0] ==0 or N4 ==0: 
                print ("Trying in body node number ")
                N1 = N.NodeByID(el[1]-treadNo)
                N2 = N.NodeByID(el[2]-treadNo)
                N3 = N.NodeByID(el[3]-treadNo)
                N4 = N.NodeByID(el[4]-treadNo)
                if N1[0] ==0 or N2[0] ==0 or N3[0] ==0 or N4 ==0: 
                    print (" > no data ")
                    continue 
...
def Plot_LoadedTireProfile(ImageName, sdb, sdbresult, simcondition,  dpi=200, ptn=None, **args):
    offset = 10000
    treadno =  10000000
    mesh = ''
    sidewave = 0
...
def Plot_LoadedTireProfile_PatternTire(ImageName, sdb, sdbresult, simcondition,  dpi=200, ptn=None, **args):
    offset = 10000
    treadno =  10000000
    mesh = ''
    sidewave = 0
    ...
    allnodes = np.array(Node.Node)
    idx = np.where(allnodes[:,0] <treadno )[0]
    npn = allnodes[idx]
    Node = NODE()
    for ix in idx: 
        Node.Add(npn[ix])
    ...
    rmax = np.max(npn[:,3])
    ix = np.where(npn[:,3] == rmax)[0][0]
    TopNode = npn[ix]

    MaxID = np.max(npn[:,0])
    Sectors = int(MaxID / offset) + 1

    Topsector = int(TopNode[0] / offset) + 1
    Bottomsector = Topsector + int(Sectors / 2 )
    if Bottomsector > Sectors:
        Bottomsector -= Sectors
...
def PlotFootprint(imagenamebase, lastSDB, lastSFRIC, mesh2d='', **args):
    ...
    if iteration == 1:   
        ...
        try: 
            ...
            fNode = np.array(FNode.Node)
            for k, pnd in enumerate(NodeContact.Node):
                ix = np.where(fNode[:,0] == pnd[0])[0]
                if len(ix): 
                    NodeContact.Node[k].append(fNode[ix[0]][4])
                else: 
...
def SimpleReadingPtn(filename): 
    Estart = 0
    Nstart = 0
    pitchscaling = 1.0  
    ModelGD = 0.0 
    TreadDesignWidth = 0.0
    ...
    for line in lines:
        ...
        if "*" in line: 
            ...
            if cmd == 'nd': 
                data = line.split(",")
                Node.append([float(int(data[0].strip()) + Nstart),  round(float(data[1].strip()) * pitchscaling, 7), \
                                round(float(data[2].strip()) * pitchscaling, 7),  round(float(data[3].strip()) * pitchscaling, 7)])

            if cmd == 'c8': 
                data = line.split(",")
                
                Solid.append([int(data[0].strip()) + Estart, \
                    int(data[1].strip()) + Nstart, int(data[4].strip()) + Nstart, \
                    int(data[3].strip()) + Nstart, int(data[2].strip()) + Nstart, \
                    int(data[5].strip()) + Nstart, int(data[8].strip()) + Nstart, \
                    int(data[7].strip()) + Nstart, int(data[6].strip()) + Nstart, 8])
            if cmd == 'c6': 
                data = line.split(",")
                # int(data[0].strip()) + Estart
                Solid.append([int(data[0].strip()) + Estart, \
                    int(data[1].strip()) + Nstart, int(data[3].strip()) + Nstart, \
                    int(data[2].strip()) + Nstart, int(data[4].strip()) + Nstart, \
                    int(data[6].strip()) + Nstart, int(data[5].strip()) + Nstart, \
                    0, 0, 6])


    return np.array(Node), np.array(Solid)
...
def ISLM_VersionCheck(sns='', solver='smart', smartlog='', output='', post='1.00'): 

    ## Pre-Version 
    meshfile = ''
    listSNS = sns.split("/")
    N = len(listSNS)
    for i, txt in enumerate(listSNS): 
        if i == N-1: break 
        meshfile += txt +"/"

    listSNS = listSNS[-1].split("-")
    N = len(listSNS)
    for i, txt in enumerate(listSNS): 
        if i ==0: continue 
        if i >= N-2: break 
        meshfile += txt +"-"
    meshfile = meshfile[:-1] + ".inp"

    with open (meshfile) as F: 
        line = F.readlines()[0]
    if 'STANDARD MESH VERSION' in line and 'v2020' in line.lower(): 
        preM = 10
    else:
        preM =0 
    preS = 0

    pre = '/home/users/fiper/ISLM/ISLMCommands/SART/SMART/Pre3DModelGeneration.py'
    with open(pre) as F: 
        lines = F.readlines()
    for line in lines: 
        if "Pre3DModelGeneration.py" in line and "##" in line: 
    ...
    if solver.lower() == 'smart': 
        pre = '/home/users/fiper/ISLM/ISLMCommands/SART/SMART/PreSMART.py'
        with open(pre) as F: 
            lines = F.readlines()
        for line in lines: 
            if "PreSMART.py" in line and "##" in line: 
                data = line.split("=")[1].strip()
                break 
        version = data.split(".")
        preM += int(version[0])
        ...
        pre = '/home/users/fiper/ISLM/ISLMCommands/SART/Static/PreABAQUSInpGeneration.py'
        with open(pre) as F: 
            lines = F.readlines()
        for line in lines: 
            if "PreABAQUSInpGeneration.py" in line and "##" in line: 
                data = line.split("=")[1].strip()
                break 
        version = data.split(".")
        preM += int(version[0])
    ...
    if solver.lower()=='abaqus': 
        fverion = '/home/users/fiper/ISLM/ISLMCommands/SART/Static/version_Abaqus.txt'
        with open(fverion) as f: 
            line = f.readlines()[0]
        solver = line.split("=")[1].strip()
    else: 
        with open(smartlog) as f: 
            lines = f.readlines()
        isversion = 0 
        for line in lines: 
            if 'DOUBLE Precision SMART' in line and 'Developed' in line and not 'Beta' in line: 
                isversion = 1 
                words = line.split("(")[0].strip()
                try: 
                    solver = words.split("v")[1].strip()
                except: 
                    print (words)
                    sys.exit()
